...
def die(playSurface,score):
    channel=pygame.mixer.Channel(3)  # create a new channel
...
def main():
    ...
    pygame.init()  # initialize the audio part
    pygame.mixer.music.load('snackmusic.mp3')  # load mp3
    pygame.mixer.music.play(-1)  # loop the background music
    channel=pygame.mixer.Channel(2)  # create a new channel
    sound=pygame.mixer.Sound('get.wav')  # create a sound object with get.wav
    fpsClock = pygame.time.Clock()
    # create the surface of the pygame
    playSurface = pygame.display.set_mode((640,480))
    pygame.display.set_caption('The New Snake Game')
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_RIGHT :  # listen to the RIGHT ARROW
                    nextorder = 'right'
                if event.key == K_LEFT :  # listen to the LEFT ARROW
                    nextorder = 'left'
                if event.key == K_UP :  # listen to the UP ARROW
                    nextorder = 'up'
                if event.key == K_DOWN :  # listen to the DOWN ARROW
                    nextorder = 'down'
                if event.key == K_ESCAPE:  # listen to the ESC
